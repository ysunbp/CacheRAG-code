...
if __name__ == "__main__":
    ...
    with open('./CWQ/questions.pkl', 'rb') as file:
        questions = pickle.load(file)
    with open('./CWQ/answers.pkl', 'rb') as file:
    ...
    relation_path = "./CWQ/properties.json"
    relation2link_dict, link2relation_dict = read_properties(relation_path)
    outpath = "./json/CWQ-cacherag-autogen.json"
    cacherag(question_set, wikisp_generator, wikisp_tokenizer, generator, outpath, relation2link_dict, openai_client)

# python -m torch.distributed.run --nproc_per_node 1 CacheRAG-CWQ-update-neighbor-cache.py >> /export/data/ysunbp/KBQA/KB-select/scripts/exp/logs/cwq/deepseek-update-one-hop-autogen.txt